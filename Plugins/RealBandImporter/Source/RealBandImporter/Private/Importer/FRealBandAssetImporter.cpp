...
//		if (fs::status(iter->path()).permissions() == fs::perms::owner_read)
		{
			...
			const std::wstring fname = iter->path().filename().wstring();
	...
	//         fname += iter->path().extension().string();
		   ...
		   // std::string substr = fname.substr(pos + 1, fname.length());
			...
			std::string fstr(fname.begin(), fname.end());
			//if (std::regex_match(fname, ext_pattern))
			if (std::regex_match(fstr, ext_pattern))
...
//	if (AssetCount > 0)
//		return;
	if (iAssetFolder.ToString().GetCharArray().GetData())
	{
		if (!iAssetFolder.IsEmpty() && std::filesystem::exists(std::filesystem::path(iAssetFolder.ToString().GetCharArray().GetData())))
		{
			...
			try
			{
				//	if (iAssetFolder.ToString().GetCharArray().GetData())
				{
					...
					{
						if (dirEntry.exists() && dirEntry.path().extension() == ".png") {

							if (dirEntry.path().filename().wstring().find(L"perspective") != std::wstring::npos
			...
			{
				FText DialogText = FText::Format(
					LOCTEXT("PluginButtonDialogText", "Unhandled Exception"),
					FText::FromString(TEXT("Unhandled Exception")),
					FText::FromString(TEXT("Unhandled Exception"))
				);
				FMessageDialog::Open(EAppMsgType::OkCancel, DialogText);
				UE_LOG(LogRealBand, Error, TEXT("Unhandled Exception "));
			}
...
void FRealBandAssetImporter::ImportTextures(const std::set<std::wstring>& iFileList)
{
	...
	if (TexFactory)
	{
		...
		for (const wstring& AssetName : iFileList)
		{
			...
			FString Name = ObjectTools::SanitizeObjectName(FPaths::GetBaseFilename(win_dir.wstring().c_str()));
			...
			if (Pkg)
			{
				ImportAssetUsingFactory(TexFactory, Name, Pkg, win_dir.wstring().c_str(), TextureList);
			}
			else
			{
	                                                ...
	                                                , TArray<UObject*> & oTextureList)
{
	...
	try
	{
		...
		if (ImportFactory->FactoryCanImport(ImportPath))
		{
			if (FName(*ImportPath).GetDisplayNameEntry()->IsWide())
			{
				WIDECHAR NewTextureNameString[NAME_SIZE] = { 0 };
				FName ConvertedFString = FName(*ImportPath);
				ConvertedFString.GetPlainWIDEString(NewTextureNameString);
				FString ImportPathWide(NewTextureNameString, NAME_SIZE );
				//const FString ImportPathWide = FString::FromInt((int32)*NewTextureNameString);
				TextureObj = static_cast<UTexture2D*>(ImportFactory->FactoryCreateFile(ImportAssetType, Pkg, *ObjectName,
					RF_Public | RF_Standalone | RF_Transactional,
					ImportPathWide, nullptr, GWarn, bUserCanceled));
			...
			else
			{
				TextureObj = static_cast<UTexture2D*>(ImportFactory->FactoryCreateFile(ImportAssetType, Pkg, *ObjectName,
					RF_Public | RF_Standalone | RF_Transactional,
					ImportPath, nullptr, GWarn, bUserCanceled));
			}
		...
		{
			UE_LOG(LogRealBand, Error, TEXT("TextureFactory cannot import  %s"), *ImportPath);
		}
	...
	{
		UE_LOG(LogRealBand, Error, TEXT("Unhandled exception ImportAssetUsingFactory"));
	}
}
...
void FRealBandAssetImporter::ImportFbx(const TArray<FString> & iFbxList, bool bObj)
{
	...
	else
	{
		...
		//UClass* objectClass = ImportedObjects[0]->GetClass();
		//UMaterial* material = ImportedObjects[0]->GetOutermostObject;
		...
		//	UE_LOG(LogRealBand, Warning, TEXT("Property Name: %s, Property Value: %s"), *prop->GetName(), *prop->GetCPPType());
		//}
